...
rl.on('line', lineString => {
  const colimns = lineString.split(',');
  const year = parseInt(colimns[0]);
  const prefecture = colimns[1];
  const popu = parseInt(colimns[3]);
...
rl.on('close',() => {
  for( const [key, value] of prefectureDataMap){ //for ( 変数 of 配列 )
